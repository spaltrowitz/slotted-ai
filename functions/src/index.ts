...
// ---------------------------------------------------------------------------
// Event Search — SeatGeek + Ticketmaster
// ---------------------------------------------------------------------------

interface EventSearchOpts {
  city?: string;
  dateFrom?: string;
  dateTo?: string;
  type?: string;
...
async function searchSeatGeek(query: string, opts: EventSearchOpts): Promise<NormalizedEvent[]> {
  ...
  try {
    ...
    if (opts.city) params.append("venue.city", opts.city);
    if (opts.type) params.append("type", opts.type);
    if (opts.dateFrom) params.append("datetime_local.gte", opts.dateFrom);
    if (opts.dateTo) params.append("datetime_local.lte", opts.dateTo);

    const resp = await fetch(`https://api.seatgeek.com/2/events?${params}`);
    if (!resp.ok) {
      console.error("SeatGeek error:", resp.status, await resp.text());
      return [];
    }
    ...
    return (data.events || []).map((ev: any) => ({
      ...
      datetimeLocal: ev.datetime_local,
      url: ev.url,
      imageUrl: ev.performers?.[0]?.image,
      priceMin: ev.stats?.lowest_price,
      priceMax: ev.stats?.highest_price,
      performers: (ev.performers || []).map((p: any) => p.name),
    }));
  } catch (err) {
...
async function searchTicketmaster(query: string, opts: EventSearchOpts): Promise<NormalizedEvent[]> {
  ...
  try {
    const params = new URLSearchParams({
      keyword: query,
      apikey: apiKey,
      size: String(opts.perPage || 20),
      sort: "date,asc",
    });
    if (opts.city) params.append("city", opts.city);
    if (opts.dateFrom) params.append("startDateTime", new Date(opts.dateFrom).toISOString().replace(".000Z", "Z"));
    if (opts.dateTo) params.append("endDateTime", new Date(opts.dateTo).toISOString().replace(".000Z", "Z"));
    if (opts.type) {
      ...
      const segment = segmentMap[opts.type];
      if (segment) params.append("classificationName", segment);
    }

    const resp = await fetch(`https://app.ticketmaster.com/discovery/v2/events.json?${params}`);
    if (!resp.ok) {
      console.error("Ticketmaster error:", resp.status, await resp.text());
      return [];
    }
    const data = await resp.json();
    ...
    return events.map((ev: any) => {
      ...
      return {
        ...
        datetimeLocal: ev.dates?.start?.localDate + "T" + (ev.dates?.start?.localTime || "19:00:00"),
        url: ev.url,
        imageUrl: ev.images?.[0]?.url,
        priceMin: priceRange?.min,
        priceMax: priceRange?.max,
        performers: (ev._embedded?.attractions || []).map((a: any) => a.name),
      };
    });
  } catch (err) {
...
function deduplicateEvents(events: NormalizedEvent[]): NormalizedEvent[] {
  ...
  for (const ev of events) {
    const dateKey = ev.datetimeLocal?.slice(0, 16) || ev.datetime?.slice(0, 16) || "";
    const key = `${ev.title.toLowerCase().replace(/[^a-z0-9]/g, "").slice(0, 30)}_${dateKey}`;
    if (!seen.has(key)) {
      seen.set(key, ev);
    }
  }
...
// GET /events/search — search SeatGeek + Ticketmaster
app.get("/events/search", requireAuth, async (req: AuthRequest, res: Response) => {
  const { q, city, dateFrom, dateTo, type } = req.query as Record<string, string>;
  if (!q) {
    res.status(400).json({ error: "Query parameter 'q' is required" });
    return;
  }
  try {
    const opts: EventSearchOpts = { city, dateFrom, dateTo, type, perPage: 20 };
    const [sgEvents, tmEvents] = await Promise.all([
      searchSeatGeek(q, opts),
      searchTicketmaster(q, opts),
    ]);
    const merged = deduplicateEvents([...sgEvents, ...tmEvents]);
    merged.sort((a, b) => {
      const da = a.datetimeLocal || a.datetime || "";
      const db = b.datetimeLocal || b.datetime || "";
...
// POST /events/match — match events with friends' availability
app.post("/events/match", requireAuth, async (req: AuthRequest, res: Response) => {
  const { query, friendId, friendIds, city, type, dateFrom, dateTo } = req.body;
  if (!query) {
    res.status(400).json({ error: "'query' is required" });
    return;
  }
  if (!friendId && (!friendIds || friendIds.length === 0)) {
    res.status(400).json({ error: "'friendId' or 'friendIds' is required" });
    return;
  }
  try {
    const me = await getDbUser(req.uid!);
    ...
    const opts: EventSearchOpts = { city, dateFrom, dateTo, type, perPage: 30 };
    const [sgEvents, tmEvents] = await Promise.all([
      searchSeatGeek(query, opts),
      searchTicketmaster(query, opts),
    ]);
    const allEvents = deduplicateEvents([...sgEvents, ...tmEvents]);
    if (allEvents.length === 0) {
      res.json({ events: [], matches: [], message: "No events found for that search." });
      return;
    }

    const participantIds: string[] = friendIds || [friendId];
    const now = new Date().toISOString();
    const sb = getSupabase();

    const slotQueries = [
      sb.from("availability").select("*").eq("user_id", me.id).eq("status", "free").gte("end_time", now).order("start_time"),
      ...participantIds.map((fid: string) =>
        sb.from("availability").select("*").eq("user_id", fid).eq("status", "free").gte("end_time", now).order("start_time")
      ),
    ];
    const slotResults = await Promise.all(slotQueries);
    const allFreeSlots = slotResults.map((r) => r.data || []);
    ...
    const matches: (NormalizedEvent & { availabilityScore: number; note: string })[] = [];
    for (const event of allEvents) {
      const eventStart = new Date(event.datetimeLocal || event.datetime);
      const durationMin = durationMap[event.type] || 120;
      const eventEnd = new Date(eventStart.getTime() + durationMin * 60000);

      let allFree = true;
      for (const participantSlots of allFreeSlots) {
        const hasCovering = participantSlots.some((slot: any) => {
      ...
      if (allFree) {
        const isWeekend = eventStart.getDay() === 0 || eventStart.getDay() === 6;
        const hour = eventStart.getHours();
        const isMatinee = hour < 17;
        let score = 70;
        if (isWeekend) score += 10;
        if (isMatinee) score += 5;
        const daysOut = (eventStart.getTime() - Date.now()) / 86400000;
        if (daysOut <= 7) score += 10;
        else if (daysOut <= 14) score += 5;
        score = Math.min(score, 100);
        matches.push({
          ...event,
          availabilityScore: score,
          note: `${isMatinee ? "Matinee" : "Evening"} show${isWeekend ? " (weekend)" : ""} — everyone is free!`,
        });
      }
    }

    matches.sort((a, b) => b.availabilityScore - a.availabilityScore);
    res.json({
      events: allEvents,
      matches,
      participantCount: participantIds.length + 1,
      message: matches.length > 0
        ? `Found ${matches.length} showtime${matches.length !== 1 ? "s" : ""} when everyone is free!`
        : `Found ${allEvents.length} event${allEvents.length !== 1 ? "s" : ""}, but no showtimes match everyone's availability in the next 2 weeks. Try expanding your date range or syncing calendars.`,
    });
  } catch (err) {
    console.error("Event match failed:", err);
    res.status(500).json({ error: "Event matching failed" });
  }
});
