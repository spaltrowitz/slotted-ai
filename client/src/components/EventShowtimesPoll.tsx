...
export default function EventShowtimesPoll({
  ...
  // Privacy: derive pending-friend names from the invited friend list, NOT
  // from server-supplied per-name availability data (which we no longer return).
  const pendingFriends = useMemo(() => [...friendNames], [friendNames]);
            ...
            {orderedShowtimes[0]?.ticketUrl && (
              <>
                ...
                <a
                  ...
                  className="inline-flex items-center gap-0.5 font-medium text-sky-600 hover:text-sky-800"
                >
                  🎟️ Tickets
                </a>
              </>
