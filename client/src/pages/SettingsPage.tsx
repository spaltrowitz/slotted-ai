            ...
            <div className="space-y-1.5">
              ...
              <div className="flex items-center justify-between py-1">
                <div className="flex items-center gap-2 min-w-0">
                  ...
                  {googleCalendarConnected && !googleCalendarStale ? (
                    <span className="inline-flex items-center gap-1 rounded-full border border-emerald-200 bg-emerald-50 px-1.5 py-0.5 text-xs font-medium text-emerald-700">
                      <span className="h-1 w-1 rounded-full bg-emerald-500 animate-pulse" />
                      Connected
                    </span>
                  ) : googleCalendarStale ? (
                    <span className="inline-flex items-center rounded-full border border-amber-200 bg-amber-50 px-1.5 py-0.5 text-xs font-medium text-amber-700">
                      Reconnect
                    </span>
                  ) : null}
                </div>
              ...
              {showAppleConnect && !appleCalendarConnected && (
                <div className="ml-8 rounded-xl border border-gray-200 bg-gray-50/50 p-3 space-y-2">
                  ...
                  {showAppleWhy && (
                    <div className="rounded-lg border border-blue-100 bg-blue-50/50 p-2 text-xs text-gray-500 space-y-1">
              ...
              <div className="rounded-2xl border border-gray-200/60 bg-white p-3 sm:p-4 shadow-sm">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                  {[
                    ...
                    <button key={opt.value} onClick={() => setSocialRecharge(opt.value)} className={`rounded-xl border px-2 py-2 text-center transition-all ${socialRecharge === opt.value ? 'border-slotted-400 bg-gradient-to-r from-slotted-50 to-indigo-50 shadow-sm' : 'border-gray-200 hover:border-slotted-200 hover:bg-gray-50'}`}>
                      <p className={`text-xs font-semibold leading-tight ${socialRecharge === opt.value ? 'text-slotted-700' : 'text-gray-600'}`}>{opt.label}</p>
                    </button>
                  ))}
                </div>
