...
export default function SettingsPage() {
  ...
  const handleSave = () => {
    console.log('Settings saved:', { travelBuffer, tripBuffer, personalTimeMode, socialFrequency, preferredTimes, callTimes, callDuration });
  ...
  const toggleCallTime = (value: string) => {
    setCallTimes((prev) =>
      prev.includes(value) ? prev.filter((t) => t !== value) : [...prev, value]
    );
  };
  ...
  const SectionHeader = ({ icon, number, title, subtitle }: { icon: string; number: number; title: string; subtitle: string }) => (
    <div className="flex items-start gap-3 mb-5">
      <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-slotted-50 to-purple-50 text-xl">
        {icon}
      </div>
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          <span className="inline-flex items-center justify-center h-5 w-5 rounded-full bg-slotted-100 text-[10px] font-bold text-slotted-700">{number}</span>
      ...
      <div className="mx-auto max-w-2xl space-y-6">
        ...
        <div className="rounded-2xl border border-gray-200/60 bg-white p-6 shadow-sm">
          ...
          <div className="flex items-center gap-4">
            {user?.photoURL ? (
              <img src={user.photoURL} alt="" className="h-12 w-12 rounded-full ring-2 ring-slotted-100" />
            ) : (
              <div className="flex h-12 w-12 items-center justify-center rounded-full bg-gradient-to-br from-slotted-400 to-purple-500 text-lg font-bold text-white">
                {user?.displayName?.[0] ?? '?'}
              </div>
            )}
            <div className="flex-1 min-w-0">
            ...
            <div className="flex gap-2">
              <button
                ...
                className="rounded-xl border border-gray-200 px-3 py-1.5 text-xs font-medium text-gray-600 transition-all hover:border-gray-300 hover:bg-gray-50"
              >
                Switch
              </button>
              <button
                onClick={signOut}
                className="rounded-xl border border-red-100 bg-red-50/50 px-3 py-1.5 text-xs font-medium text-red-600 transition-all hover:bg-red-50 hover:border-red-200"
              >
                Sign out
              </button>
            </div>
          ...
          <div className="mt-5 border-t border-gray-100 pt-5">
            {calendarConnected ? (
              <>
                <div className="flex items-center justify-between rounded-xl border border-emerald-100 bg-gradient-to-r from-emerald-50/50 to-teal-50/50 px-4 py-3">
              ...
              <>
                <div className="flex items-center justify-between rounded-xl border border-amber-100 bg-gradient-to-r from-amber-50/50 to-orange-50/50 px-4 py-3">
                  <div className="flex items-center gap-3">
                    <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-white shadow-sm">
                      {googleIcon}
                    </div>
        ...
        <div className="rounded-2xl border border-gray-200/60 bg-white p-6 shadow-sm">
          ...
          <div>
            <label className="block text-xs font-medium uppercase tracking-wider text-gray-400 mb-3">
              When are you available to hang out?
            </label>
          ...
          <div className="mt-5 border-t border-gray-100 pt-5">
            ...
            <p className="mt-0.5 text-[11px] text-gray-400">Padding before/after each in-person meetup so you're never rushing</p>
            <select
              value={travelBuffer}
          ...
          <div className="mt-5 border-t border-gray-100 pt-5">
            ...
            <p className="text-[11px] text-gray-400 mb-3">When we detect travel on your calendar, should we auto-block recovery time?</p>
            <div className="space-y-2">
              {[
        ...
        <div className="rounded-2xl border border-gray-200/60 bg-white p-6 shadow-sm">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
              {[
                ...
                return (
                  <button
                    key={opt.value}
                    onClick={() => toggleCallTime(opt.value)}
                    className={`rounded-xl border px-3 py-2.5 text-left text-xs transition-all ${
                      selected
                        ? 'border-blue-400 bg-gradient-to-r from-blue-50 to-cyan-50 text-blue-700 shadow-sm'
            ...
            {callTimes.length === 0 && (
              <p className="mt-2 text-[11px] text-gray-400">No call windows selected yet — select at least one if you're open to calls</p>
            )}
          </div>
          ...
          <div className="mt-5 border-t border-gray-100 pt-5">
            ...
            <p className="mt-0.5 text-[11px] text-gray-400">How long do you usually want to chat?</p>
            <div className="mt-2 flex gap-2">
              {[
                { value: '15', label: '15 min', desc: 'Quick catch-up' },
                { value: '30', label: '30 min', desc: 'Good chat' },
                { value: '60', label: '1 hour', desc: 'Deep catch-up' },
                { value: 'open', label: 'No limit', desc: 'Go with the flow' },
              ].map((opt) => (
                <button
                  key={opt.value}
                  onClick={() => setCallDuration(opt.value)}
                  className={`flex-1 rounded-xl border px-2 py-2.5 text-center transition-all ${
                    callDuration === opt.value
                      ? 'border-blue-400 bg-gradient-to-r from-blue-50 to-cyan-50 text-blue-700 shadow-sm'
                      : 'border-gray-200 text-gray-600 hover:border-blue-200 hover:bg-gray-50'
                  }`}
                >
