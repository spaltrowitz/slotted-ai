...
export default function EventInviteLandingPage() {
  ...
  if (phase === 'vote' && schedule) {
    const showtimes = schedule.showtimes
      .map((showtime, originalIndex) => ({
        ...
        showtime: {
          ...
          available: false,
          allFree: votersByShowtime[originalIndex] || [],
          conflicts: [],
    ...
    const bestOverlapCount = showtimes.reduce((best, item) => Math.max(best, item.showtime.allFree.length), 0);
      ...
      <div className="min-h-screen bg-gradient-to-br from-purple-50 via-white to-amber-50 px-4 py-6">
        <div className="mx-auto max-w-md space-y-4">
          ...
          <div className="space-y-2.5 pb-28">
            {showtimes.map(({ showtime, originalIndex }) => (
              <div key={`${showtime.datetime}-${originalIndex}`} className="space-y-1">
                {allResponsesIn && showtime.allFree.length === bestOverlapCount && (
